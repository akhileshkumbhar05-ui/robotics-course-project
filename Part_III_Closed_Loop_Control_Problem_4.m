...
%% Solve ODE
[t, x] = ode23(@(t,x) arm2link_pd_gravity_ode(t, x, params), tspan, x0);
...
for i = 1:length(t)
    ...
    tau = params.Kp*e + params.Kv*de + G;
...
function dx = arm2link_pd_gravity_ode(t, x, params)
...
function G = gravity_vector(q, params)
    ...
    G1 = (m1 + m2)*g*a1*cos(theta1) + m2*g*a2*cos(theta1 + theta2);
