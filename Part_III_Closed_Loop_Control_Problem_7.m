...
%% PID computed-torque gains
params.Kp = diag([2500 2500]);
params.Kv = diag([100 100]);
params.Ki = diag([200 200]);
...
%% Simulation settings
tspan = [0 10];
x0 = [0; 0; 1; 2; 0; 0];   % [theta1; theta2; dtheta1; dtheta2; eps1; eps2]

%% Solve ODE
[t, x] = ode23(@(t,x) arm2link_pid_ctc_sat_ode(t, x, params), tspan, x0);
...
%% Compute saturated control torques for plotting
...
for i = 1:length(t)
    ...
    v = ddqd + params.Kv*de + params.Kp*e + params.Ki*eps;
    tau_cmd = M*v + Cvec + G;

    tau = min(max(tau_cmd, params.tau_min), params.tau_max);
...
yline(15,'r--','LineWidth',1.2);
yline(-15,'r--','LineWidth',1.2);
grid on;
xlabel('Time (s)');
ylabel('Control Torque (N.m)');
title('Time vs Control Torques');
legend('\tau_1', '\tau_2', 'Upper Limit', 'Lower Limit', 'Location', 'best');
...
function dx = arm2link_pid_ctc_sat_ode(t, x, params)
% x = [theta1; theta2; dtheta1; dtheta2; eps1; eps2]
